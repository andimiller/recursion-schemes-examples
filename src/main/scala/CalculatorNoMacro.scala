...
object CalculatorNoMacro {
  ...
  object Expr {
    ...
    implicit val traverseInstance: Traverse[ExprF] = new DefaultTraverse[ExprF] {
      override def traverse[G[_], AA, B](fa: ExprF[AA])(fn: AA => G[B])(implicit A: Applicative[G]): G[ExprF[B]] = fa match {
        case NumberF(i) => A.map(A.pure(i))(NumberF.apply[B])
    ...
    def projectCoalgebra: Coalgebra[ExprF, Expr] = new GCoalgebra[ExprF, Expr, Expr]({
      case Number(i) => NumberF[Expr](i)
      case Add(l, r) => AddF[Expr](l, r)
      case Multiply(l, r) => MultiplyF[Expr](l, r)
    })

    def embedAlgebra: Algebra[ExprF, Expr] = new GAlgebra[ExprF, Expr, Expr]({
