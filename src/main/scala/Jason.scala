...
object Jason {
  ...
  object parsers {
    ...
    val arrayLiteralParser: Parser[ExprF[String]] =
      char('[') *> takeWhileNotUnbracketed(Set(',', ']'))
        .sepBy1(char(','))
        .map(xs => ArrayLiteralF[String](xs.toList).asInstanceOf[ExprF[String]]) <* char(']')
    ...
    def apply(s: String): ExprF[String] =
      List[Parser[ExprF[String]]](
        objectLiteralParser,
        arrayLiteralParser,
        stringLiteralParser,
        intLiteralParser,
